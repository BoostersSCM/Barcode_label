...
# pymysql 드라이버 필요 (requirements.txt에 pymysql 포함)
import pymysql  # noqa: F401  # SQLAlchemy가 pymysql 드라이버를 로드할 수 있도록 import
...
def load_product_data():
    ...
    if engine is None:
        st.error("ERP DB 연결이 설정되지 않았습니다. secrets를 확인하세요.")
        return pd.DataFrame()
...
def insert_inventory_record(data: dict) -> bool:
    ...
    query = """
        ...
        VALUES
        (%(serial_number)s, %(category)s, %(product_code)s, %(product_name)s, %(lot)s,
         %(expiry)s, %(disposal_date)s, %(location)s, %(version)s, %(inbound_datetime)s,
         %(status)s, %(outbound_datetime)s, %(outbound_person)s)
    """
    try:
...
def insert_inout_record(data: dict) -> bool:
    ...
    query = """
        ...
        VALUES
        (%(timestamp)s, %(type)s, %(serial_number)s, %(product_code)s,
