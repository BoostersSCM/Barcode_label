...
def add_item_to_outbound_list():
    ...
    if scanned_code.startswith('88'):
        ...
        if product_info:
            item_to_add = {
                "type": "제품", "code": scanned_code,
                "product_name": product_info.get('resource_name', 'N/A'),
                "product_code": product_info.get('resource_code', 'N/A'), "qty": 1
            }
        else:
...
if not st.session_state.outbound_list:
    ...
    for i, item in enumerate(st.session_state.outbound_list):
        ...
        with col2:
            ...
            new_qty = st.number_input("수량", min_value=1, value=item['qty'], step=1, key=f"qty_{item['code']}", disabled=is_disabled)
